...
**The output formats and results files have completely changed.  However, the characteristics of the analyses performed still holds.** 
...
Event chains are collections of events for execution at an LP that could potentially be
executed together.  Thus when examining an event with receive timestamp **t**, the chain
is formed by all future (by receive time) events in that LP that have a send time **< t**.
Three types of event chains are identified, namely: *local event chains*, *linked event
chains*, and *global event chains*.  Local chains have the additional constraint that they
must also have been sent by the executing LP (self generated events).  Global chains can
be sent from any LP.  Linked chains are similar to local chains except that the constraint
on the send time is relaxed so that any event sent within the time window of the local
event chain (so basically any event generated by the chain is also potentially a member of
the chain).  

To limit the amount of data collected, the event chain computations only identify chain
lengths of 1, 2, 3, 4, and those >= 5.  Preliminary analysis shows that few chains exceed
lengths of 5.

***These subsections will all have to be modified as we better understand how to present
   this data***
...
## Linked Event Chains by LPi 

This graph shows the minimum, average, and maximum length of the linked event chains by
LP. 

* x-axis: labeled "LPs", enumerates the names of the LPs in the model.

* y-axis: labeled "Linked Event Chain", showing (for each LP), the minimum, average, and
  maximum linked event chain for that LP.

* Computed by: outline given in section "Event Chains". 

![linked_event_chain_by_lp](./graphs/linked_event_chain_by_lp.pdf)

## Summary of Linked Event Chains

This graph shows the frequency of linked event chains of length X.

* x-axis: labeled "Linked Event Chain Length", enumerates the length of linked event
  chains seen in the simulation.

* y-axis: labeled "Frequency of Occurrence", showing the frequency of that linked event
  chain length in the entire simulation.

* Computed by: obvious.

![frequency_of_linked_event_chain_by_length](./graphs/frequency_of_linked_event_chain_by_length.pdf)
