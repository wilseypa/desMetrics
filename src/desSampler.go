...
// we will also keep a running count the event and store the first time and last time it's seen for that LP
// (for purposes of trim-until-all-lps-have-events).  store the other "companion" LP internally.
...
func main() {
	...
	// so we need to map the csv fields from the event data file to the order we need for our internal
	// data structures (sLP, sTS, rLP, rTS).  the array eventDataOrderTable will indicate which csv entry
	// corresponds; thus (for example), eventDataOrderTable[0] will hold the index where the sLP field
	// lies in desTraceData.EventData.FileFormat
	...
	// sanity check; when error generated turn on debugging and look at -1 field in eventDataOrderTable to
	// discover mssing entry
	...
	// number of LPs, and the total number of events.  on the second pass we will build the requested
	// samples. 
	...
	// record a unique int value for each LP and store the first/last traceRecord/simTime that lp occurs
	// in the event trace (traceRecord = location in the trace file, simTime = simulation time), on both
	// the sending and receiving side.
	type lpMap struct {
		firstSendRecord int
		firstSendTime float64
		firstReceiveRecord int
		firstReceiveTime float64
		lastSendRecord int
		lastSendTime float64
		lastReceiveRecord int
		lastReceiveTime float64
	}

	lpNameMap := make(map[string]*lpMap)

	// lookup LP by name, if necessary, build new entry
	lookupLP := func(lp string) *lpMap {
		item, present := lpNameMap[lp]
		if !present {
			lpNameMap[lp] = new(lpMap)
			item = lpNameMap[lp]
			item.firstSendRecord = -1
			item.firstSendTime = -1.0
			item.lastSendRecord = -1
			item.lastSendTime = -1.0
			item.firstReceiveRecord = -1
			item.firstReceiveTime = -1.0
			item.lastReceiveRecord = -1
			item.lastReceiveTime = -1.0
		}
		return item
	}
	...
	processEvent := func(sLP string, sTS float64, rLP string, rTS float64) {
		numOfEvents++
		lp := defineLP(sLP)
		if lp.firstSendRecord = -1 {
			lp.firstSendRecord = numOfEvents
			lp.firstSendTime = sTS
			lastFirstSendRecord = numOfEvents
			lastFirstSentTime = sTS
		}
		lp.lastSendRecord = numOfEvents
		lp.lastSendTime = sTS
		lp = defineLP(rLP)
		if lp.firstReceiveRecord = -1 {
			lp.firstReceiveRecord = numOfEvents
			lp.firstReceiveTime = rTS
		}
		lp.lastReceiveRecord = numOfEvents
		lp.lastReceiveTime = rTS
	}
