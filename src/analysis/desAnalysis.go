...
// On the first pass: record all new LPs into the lpNameMap and count number of sent/received
// events by each LP; count total number of events in the simulation
...
// Event processing function for the second pass over the JSON file
...
func addEvent(sLP, rLP string, sTS, rTS float64) {
	...
	if lpIndex[rLPint] > cap(lps[rLPint].events) {
		log.Fatal("Something wrong, we should have computed the appropriate size size on the first parse.\n")
	}
...
func updateRunningMeanVariance(currentMean, varianceSum, newValue float64, numValues int) (float64, float64) {
...
func processEventDataFile(process func(sLP, rLP string, sTS, rTS float64)) {
	...
	for {
		...
		if sendTime > receiveTime {
			log.Fatalf("Event has send time greater than receive time: %v %v %v %v\n",
				eventRecord[eventDataOrderTable[0]], eventRecord[eventDataOrderTable[2]], sendTime, receiveTime)
		}
		...
		process(eventRecord[eventDataOrderTable[0]], eventRecord[eventDataOrderTable[2]], sendTime, receiveTime)
	}
...
// received messages).
func numOfLPsToCover(total, percent int, data []int) int {
	numRequired := int(((float32(total) * float32(percent)) / 100.0) + .99)
...
func computeLPEventsProcessed(lp lpData) lpEventSummary {
	...
	es.cover[0] = numOfLPsToCover(es.remote, 75, lpEventSendCount)  //  75% cutoff
	es.cover[1] = numOfLPsToCover(es.remote, 80, lpEventSendCount)  //  80% cutoff
	es.cover[2] = numOfLPsToCover(es.remote, 90, lpEventSendCount)  //  90% cutoff
	es.cover[3] = numOfLPsToCover(es.remote, 95, lpEventSendCount)  //  95% cutoff
	es.cover[4] = numOfLPsToCover(es.remote, 100, lpEventSendCount) // 100% cutoff
	return es

}

func accumulateChain(chain []int, maxChainLength, chainLength int) {
	if chainLength >= maxChainLength {
		chain[maxChainLength-1]++
	} else {
		chain[chainLength]++
	}
	return
...
func computeEventChains(lp lpData) ([]int, []int, []int) {
	...
	for i < len(lp.events) {
		...
		j := i + 1
		// find end of chain
		for j < len(lp.events) && lp.events[j].companionLP == lp.lpId && lp.events[j].sendTime < lp.events[i].receiveTime {
			j++
		}
	...
	for i < len(lp.events) {
		...
		j := i + 1
		// find end of chain
		for j < len(lp.events) && lp.events[j].companionLP == lp.lpId && lp.events[j].sendTime <= lp.events[j-1].receiveTime {
			j++
		}
	...
	for i < len(lp.events) {
		j := i + 1
		for j < len(lp.events) && lp.events[j].sendTime < lp.events[i].receiveTime {
			j++
		}
...
// this will be invoked as a goroutine with LPs equally (nearly) partitioned among them
func analyzeReceivedEvents(lps []lpData, c chan<- lpEventSummary) {
	for _, lp := range lps {
...
func analyzeEventsByReceiveTime() {
	...
	for _, lp := range lps {
		...
		for _, event := range lp.events {
			...
			if event.receiveTime == lastEventTime {
				...
				if delta <= 0.0 {
					log.Fatal("Something is amiss; it should not be possible to have a receive time delta (%v) of zero or less.\n", delta)
				}
	...
	allSamplesLoop:
		for {
			...
			if sampleCount == numEventDeltaSamples {
				break
			}
		...
		oneSampleLoop:
			for {
				...
				if (start + count) > (len(lp.events) - 1) {
					continue allSamplesLoop
				}
...
func analyzeSimCycle(lps []lpData, scheduleTime float64, definingLP int) simCycleAnalysisResults {
...
func analyzeSimulationCycle(lps []lpData, c1 <-chan simCycleAnalysisResults, c2 chan<- simCycleAnalysisResults) {
	for {
		nextCycle := <-c1
		if nextCycle.eventsExhausted == true {
			close(c2)
			return
		} // done
...
func main() {
	...
	for _, entry := range eventDataOrderTable {
		if entry == -1 {
			log.Fatal("Missing critcal field in event_data->format of model json file; run with --debug to view relevant data.\n")
		}
	}
	...
	processEventDataFile(processEvent)
	...
	processEventDataFile(addEvent)

	// build the reverse map: integers -> LP Names
	mapIntToLPName := make([]string, numOfLPs)
	for key, value := range lpNameMap {
		mapIntToLPName[value.toInt] = key
	}
	...
	for i := range lps {
		...
		if maxLPSentArray < lps[i].sentEvents {
			maxLPSentArray = lps[i].sentEvents
		}
	}
	...
	for i := range lps {
		...
		if maxLPEventArray < len(lps[i].events) {
			maxLPEventArray = len(lps[i].events)
		}
	}
	...
	for i := range lps {
		sort.Sort(byReceiveTime(lps[i].events))
	}
	...
	// --------------------------------------------------------------------------------
	// create a json formatted config file in the output directory, and overwrite the file
	// in the current directory to provide correct counts on LPs/events

	outFile, err := os.Create("analysisData/modelSummary.json")
	check(err)
	outFile2, err := os.Create("modelSummary.json")
	check(err)
	...
	jsonEncoder = json.NewEncoder(outFile2)
	jsonEncoder.SetIndent("", "\t")
	err = jsonEncoder.Encode(&desTraceData)
	err = outFile.Close()
	check(err)
	...
	for range lps {
		...
		for _, i := range eventsProcessed.cover {
			fmt.Fprintf(numToCover, ", %v", i)
		}
	...
	if commSwitchOff == false {
		...
		for j, lp := range lps {
			...
			for _, event := range lp.events {
				...
				if (minTimeDelta[event.companionLP] > delta) && (event.receiveTime != 0.0) && (event.sendTime > 0.0) {
					minTimeDelta[event.companionLP] = delta
				}
				if maxTimeDelta[event.companionLP] < delta {
					maxTimeDelta[event.companionLP] = delta
				}
	...
	for {
		...
		if maxEventsAvailable < nextCycle.numAvailable {
			maxEventsAvailable = nextCycle.numAvailable
		}
		simCycle++
	}
