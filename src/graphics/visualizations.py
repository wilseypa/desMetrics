    ...
    for yAxisType, plotType in itertools.product(('linear', 'log'), ('line', 'scatter')) :
        ...
        for i in range(len(data)) :
            ...
            if xRange > len(data[i]) :
                print "Insufficient data to support plotting points in the requested range " + sampleNames[i] + ". Desired Range/data available: " + str(xRange) + "/" + str(len(data[i]))
                continue
            if xRange == 0: 
                xIndex = np.arange(len(data[i]))
            else: 
                xIndex = np.arange(len(data[i])).astype(float)/float(len(data[i]))*xRange
            if plotType == 'line' :
...
def total_events_processed(modelDirs) :
    ...
    print "    Creating Summary Graphic of All Models (eventsProcessedNormalizedToMax)"
    ...
    for index in range(len(modelNames)) :
        ...
        print "    Working on normalizedAverageTimestampDelta for: " + modelDirs[index,0]
